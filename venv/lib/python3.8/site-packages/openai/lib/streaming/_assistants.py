...
class AssistantEventHandler:
    ...
    def _emit_sse_event(self, event: AssistantStreamEvent) -> None:
        ...
        self.__current_message_snapshot, new_content = accumulate_event(
            event=event, current_message_snapshot=self.__current_message_snapshot,
        )
        ...
        accumulate_run_step(
            event=event, run_step_snapshots=self.__run_step_snapshots,
        )
        ...
        if (
            ...
            self.on_run_step_delta(
                event.data.delta, step_snapshot,
            )
        elif (
...
def accumulate_run_step(
    *, event: AssistantStreamEvent, run_step_snapshots: dict[str, RunStep],
) -> None:
...
def accumulate_event(
    *, event: AssistantStreamEvent, current_message_snapshot: Message | None,
