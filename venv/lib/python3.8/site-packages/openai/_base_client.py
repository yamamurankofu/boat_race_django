...
class AsyncPaginator(Generic[_T, AsyncPageT]):
    ...
    async def _get_page(self) -> AsyncPageT:
        def _parser(resp: AsyncPageT) -> AsyncPageT:
            resp._set_private_attributes(
                model=self._model, options=self._options, client=self._client,
            )
            return resp
    ...
    async def __aiter__(self) -> AsyncIterator[_T]:
        ...
        page = cast(
            AsyncPageT, await self,  # type: ignore
        )
...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def _validate_headers(
        self, headers: Headers, custom_headers: Headers,  # noqa: ARG002  # noqa: ARG002
    ) -> None:
...
class SyncAPIClient(BaseClient[httpx.Client, Stream[Any]]):
    ...
    def _prepare_options(
        self, options: FinalRequestOptions,  # noqa: ARG002
    ) -> FinalRequestOptions:
        """Hook for mutating the given options"""
        return options

    def _prepare_request(self, request: httpx.Request,) -> None:  # noqa: ARG002
    ...
    def _request_api_list(
        self, model: Type[object], page: Type[SyncPageT], options: FinalRequestOptions,
    ) -> SyncPageT:
        def _parser(resp: SyncPageT) -> SyncPageT:
            resp._set_private_attributes(
                client=self, model=model, options=options,
            )
            return resp
...
class AsyncAPIClient(BaseClient[httpx.AsyncClient, AsyncStream[Any]]):
    ...
    async def _prepare_request(self, request: httpx.Request,) -> None:  # noqa: ARG002
        """This method is used as a callback for mutating the `Request` object
        after it has been constructed.
        This is useful for cases where you want to add certain headers based off of
        the request properties, e.g. `url`, `method` etc.
...
Platform = Union[
    OtherPlatform,
    Literal[
        "MacOS", "Linux", "Windows", "FreeBSD", "OpenBSD", "iOS", "Android", "Unknown",
    ],
]
...
def _merge_mappings(
    obj1: Mapping[_T_co, Union[_T, Omit]], obj2: Mapping[_T_co, Union[_T, Omit]],
) -> Dict[_T_co, _T]:
